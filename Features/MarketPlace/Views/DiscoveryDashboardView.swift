    ...
    var body: some View {
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    VStack(alignment: .leading) {
                        ...
                        if viewModel.filteredRecommendedLots.isEmpty {
                            ...
                            ScrollView(.horizontal, showsIndicators: false) {
                                HStack(spacing: 16) {
                                    ForEach(viewModel.filteredRecommendedLots) { lot in
                    ...
                    VStack(alignment: .leading) {
                        HStack {
                            Text("Sellers in Radius")
                                .font(.title3.bold())
                            Spacer()
                            Text("\(viewModel.filteredLots.count) Found")
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                        .padding(.horizontal)
                        
                        if viewModel.filteredLots.isEmpty {
                            VStack(spacing: 8) {
            ...
            .sheet(isPresented: $viewModel.isFullScreenMapPresented) {
                FullScreenLocationPicker(searchCenter: $viewModel.searchCenter, searchRadius: $viewModel.searchRadius, lots: viewModel.generalLots)
            }
        }
    }
}
