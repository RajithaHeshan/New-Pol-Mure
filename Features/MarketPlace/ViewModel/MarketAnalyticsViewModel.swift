...
class MarketAnalyticsViewModel {
    ...
    // Raw bids + seller locations cached for re-filtering without re-fetching
    ...
    // MARK: - Sellers listener — keeps lat/lng map live
    ...
    // MARK: - Bids listener — last 14 days
    ...
    func recomputeTrend() {
        ...
        if selectedZone.id == "all" {
            ...
            bidsForZone = allBids.filter { bid in
                guard let loc = sellerLocations[bid.sellerID] else { return false }
                return selectedZone.contains(lat: loc.lat, lng: loc.lng)
            }
        }

        computeTrend(from: bidsForZone)
    }

    // MARK: - Compute 7-Day Trend from filtered bids
